...
class InternalCoordinateEncoder(nn.Module):
    ...
    def forward(self, H_embeddings, distances, distance_indices, phis, phi_indices, psis, psi_indices, node_map, LS_map, alpha_indices):
        ...
        # Create a tensor properly with current standards
        small_value = torch.tensor([1e-12], device='cuda' if torch.cuda.is_available() else 'cpu')

        # Use this tensor in your computation
        phase_shift_linear_output_normalized = phase_shift_linear_output / torch.max(phase_shift_norm, small_value)
        ...
        if self.c_coefficient_normalization:
            if self.c_coefficient_normalization == 'softmax':
                ...
                NotImplementedError('argument c_coefficient_normalization is not recognized')
...
class ChIRo_Encoder(nn.Module):
    def __init__(self, F_z_list, F_H, F_H_embed, F_E_embed, F_H_EConv, layers_dict, activation_dict, GAT_N_heads = 1, 
        ...
        super(ChIRo_Encoder, self).__init__()
